...
def calculate_max_zoom_native(gsd_m: float, lat: float) -> int:
    """
    Calculate maximum zoom level based on native raster resolution.
    
    Returns the actual native zoom level where pixels match resolution,
    with sensible bounds (z10–z28). No artificial clamping.
    
    This allows:
    • High-res drone (5 cm/px): max_zoom ≈ 22
    • Medium-res drone (10 cm/px): max_zoom ≈ 21
    • Satellite (50 cm/px): max_zoom ≈ 18
    • Each OM gets an efficient pyramid with no wasted tiles
    
    Args:
        gsd_m: Ground sample distance (metres per pixel)
        lat: Latitude in degrees (must be -90 to +90)
    
    Returns:
        Native max zoom level (between 10 and 28).
        Falls back to 22 if calculation fails.
    """
    import math
    
    # Validate lat (catch projected CRS being passed by mistake)
    if not (-90 <= lat <= 90):
        return 22  # Fallback to safe default
    
    cos_lat = math.cos(math.radians(lat))
    
    # At what zoom does 1 screen pixel = 1 raster pixel?
    # m_per_px = 156543.03 * cos(lat) / 2^z
    # Solve for z when m_per_px ≈ gsd_m
    arg = 156543.03 * cos_lat / gsd_m
    ...
    try:
        native_z = math.log2(arg)
    except (ValueError, TypeError):
        return 22  # Fallback
    
    # Return actual native zoom, with sensible bounds
    # Lower bound: z10 (entire Earth in ~4 tiles)
    # Upper bound: z28 (Web Mercator theoretical max, though z24+ rarely useful)
    # Practical range: z17–z24 covers all real-world sources
    return max(10, min(28, int(native_z)))
...
def _pure_rasterio_tile(cog_path: str, tiles_dir: Path,
    ...
    with rasterio.open(cog_path) as src:
        ...
        cos_lat    = math.cos(math.radians((s_lat + n_lat) / 2.0))
        min_zoom   = max(0, int(math.log2(156543.03 * cos_lat / (native_gsd * tile_size * 2))) - 1)
        # Go up to native resolution — no artificial cap
        actual_max = max_zoom
...
def generate_info_file(config, om_tile_entries, underlay_om_id):
    ...
    lines += [
        ...
        "     Formula: native_zoom = log₂(156543.03 × cos(latitude) / GSD)",
        "     Result : Per-OM max_zoom matching native resolution exactly.",
        "              No hardcoded z22 — each OM gets only the levels it needs.",
        "              Saves 1–4 zoom levels on coarse data (25–67% fewer tiles).",
        "",
    ...
    for om_id, (_gpkg, ortho_path, stem) in enumerate(pairs, start=1):
        ...
        if needs_tiles:
            ...
            try:
                ...
                # Detect native GSD and calculate adaptive max_zoom
                with rasterio.open(src_for_tiles) as src:
                    native_gsd = abs(src.transform.a)  # metres per pixel
                    
                    # Extract centre latitude
                    # If raster is in projected CRS (e.g. EPSG:3857), convert to lat/lon first
                    if src.crs and src.crs.to_epsg() != 4326:
                        # Raster is in projected CRS — reproject bounds to WGS84
                        from rasterio.warp import transform_bounds
                        w, s, e, n = transform_bounds(src.crs, "EPSG:4326", *src.bounds)
                        src_lat = (n + s) / 2
                    else:
                        # Raster is already in lat/lon (or unknown CRS)
                        src_lat = (src.bounds.top + src.bounds.bottom) / 2
                
                adaptive_max_zoom = calculate_max_zoom_native(native_gsd, src_lat)
                # Use the smaller of: user-specified max or adaptive max
                effective_max_zoom = min(args.max_cog_zoom, adaptive_max_zoom)
