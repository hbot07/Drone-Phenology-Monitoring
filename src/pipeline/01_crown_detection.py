...
def _resolve_predictions_dir(requested_dir: Path, workspace_dir: Path) -> Path:
    """Detectree2 may ignore the requested output dir and write to cwd/predictions."""
    candidates = [requested_dir, workspace_dir / "predictions"]
    for candidate in candidates:
        if candidate.exists() and list(candidate.glob("Prediction_*.json")):
            return candidate
...
def process_one_orthomosaic(
    ...
    if not (geo_folder.exists() and list(geo_folder.glob("*.geojson"))):
        ...
        predict_on_data(
            ...
            out_folder=str(predictions_folder),
            predictor=predictor,
        )

        predictions_folder = _resolve_predictions_dir(predictions_folder, Path.cwd())
        print(f"    Predictions dir: {predictions_folder}")
        print(f"  [GEO] {stem}")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="CPU thread count for inference (default: all visible CPU threads)",
    )
    parser.add_argument(
        "--thresholds",
        default=None,
    ...
    try:
        cfg_detectree.MODEL.DEVICE = device
    except Exception:
        pass
