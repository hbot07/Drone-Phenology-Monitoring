...
def build_records(
    ...
    for crown_index in sorted(pixel_by_index):
        ...
        # Primary: match by crown_index (both master and pixel use 0-based crown_index).
        # Fallback: label / id lookup (master uses 1-indexed labels so these will
        # disagree with the 0-indexed pixel labels — kept only as a last resort).
        master = (
            master_by_index.get(crown_index)
            or master_by_label.get(asset_label or "")
...
def main() -> int:
    ...
    default_viewer = root / "output" / "sit_pipeline_fixed" / "04_viewer"
    ...
    # Override base image / crowns geojson only when explicitly specified.
    # By default the values are taken from the manifest (which already has the
    # correct OM stem for the pipeline run that produced the viewer assets).
    parser.add_argument("--base-image", default=None)
    parser.add_argument("--crowns-geojson", default=None)
    ...
    # Only override manifest values when CLI args are explicitly supplied.
    if args.base_image:
        manifest["base_image"] = args.base_image
    if args.crowns_geojson:
