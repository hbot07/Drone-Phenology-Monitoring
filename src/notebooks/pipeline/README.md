...
python 00_discover_oms.py \
    ...
    --exclude-stems <LIST>   comma-separated stems to drop (e.g. odm_orthophoto_9_12_25)
    --only-stems <LIST>      comma-separated subset of stems to include — useful for quick test runs
                             e.g. --only-stems sit_om3,sit_om4,sit_om5
...
**Notes:**
- OMs are sorted chronologically: LHC filenames use `odm_orthophoto_D_M_YY` date format; SIT filenames use `sit_omN` sequential numbering.
- `--exclude-stems` is critical for LHC — the Dec-9 OM (`odm_orthophoto_9_12_25`) has severe misalignment and will corrupt the tracking graph if included.
...
> ⚠️ **Always exclude `odm_orthophoto_9_12_25`** (Dec-9 OM). It has severe spatial misalignment that corrupts the tracking graph.
...
python 00_discover_oms.py \
    --om-dir input/input_om_sit \
    --crowns-dir output/detectree_om_sit_multithreshold/crowns_multithreshold \
    --only-stems sit_om1,sit_om3,sit_om5 \
...
This runs in a fraction of the time of a full 14-OM run.
...
```json
{
  ...
  "om_stems": ["sit_om1", "sit_om2", ...],
  "exclude_stems": [],
  "pairs": [
    ["/path/to/crown.gpkg", "/path/to/ortho.tif", "sit_om1"],
    ...
  ],
  "tile_width": 25,
  "tile_height": 25,
