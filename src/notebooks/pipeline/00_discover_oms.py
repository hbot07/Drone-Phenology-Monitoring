...
def _parse_lhc_date(stem: str) -> Optional[date]:
    """Parse date from LHC filename pattern: odm_orthophoto{D}_{M}_{YY} or odm_orthophoto_{D}_{M}_{YY}."""
...
def discover_and_sort_stems(om_dir: Path, exclude_stems: List[str]) -> List[str]:
    ...
    # Remove exact duplicates (case-insensitive extension duplication)
    seen = set()
    unique = []
    for f in tif_files:
...
def main() -> int:
    ...
    if existing_crowns_dir:
        ...
        config["pairs"] = [
            [
                stem_to_gpkg.get(s) or str(Path(existing_crowns_dir) / f"{s}_multithreshold.gpkg"),
                str(om_dir / f"{s}.tif"),
                s,
            ]
            for s in stems
